...
Инструкция по настройке Reserved VM:
1. Выберите Machine: 0.5 vCPU / 2 GIB RAM 
2. Тип: Background Worker
3. Run command: python deploy_bot.py

Скрипт выполняет следующие операции:
- Проверяет наличие токена Telegram в переменных окружения
- Сбрасывает сессию Telegram API для избежания конфликтов
- Завершает все существующие процессы бота
- Запускает бота и монитор здоровья в отдельных процессах
- Обеспечивает постоянную работу бота, перезапуская процессы при необходимости
"""
