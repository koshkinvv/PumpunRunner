...
Ответ должен быть структурирован в виде JSON с полями:
- plan_name: название плана
- plan_description: общее описание плана
- weekly_volume: общий объем в км
- intensity_distribution: распределение интенсивности
- training_days: массив дней тренировок с полями day, day_of_week, workout_type, distance, pace, heart_rate, description, purpose
- rest_days: массив дней отдыха с полями day, day_of_week, activity, purpose
- recommendations: объект с рекомендациями (nutrition, recovery, progression)
"""
    ...
    def get_training_plan(self, profile):
        ...
        try:
            ...
            logger.info("Отправляем запрос к OpenAI для получения плана тренировок")
            ...
            logger.info("Получен ответ от API, пытаемся распарсить JSON")
            
            plan = json.loads(content)
            logger.info("План тренировок успешно получен и распарсен")
...
def test_prompt():
    ...
    try:
        ...
        filename = f'test_training_plan_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(plan, f, ensure_ascii=False, indent=2)
        
        print(f"\nТестовый план тренировок успешно создан и сохранен в файл {filename}!")
        ...
        for day in plan.get('training_days', []):
            ...
            workout = day.get('workout_type', '?')
            distance = day.get('distance', '?')
            print(f"День {day_num}: {day_name} - {workout} ({distance} км)")
        
        print("\nДни отдыха:")
        for day in plan.get('rest_days', []):
            day_num = day.get('day', '?')
            day_name = day.get('day_of_week', '?')
            activity = day.get('activity', 'Отдых')
            print(f"День {day_num}: {day_name} - {activity}")
        ...
        for key, value in recommendations.items():
            ...
            if isinstance(value, str) and len(value) > 50:
                short_val = value[:50] + "..."
            print(f"- {key.capitalize()}: {short_val}")
