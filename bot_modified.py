...
async def callback_query_handler(update, context):
    ...
    if query.data == "confirm_new_plan":
        ...
        # Создаем специальный хэндлер для обновления профиля через команду
        
        # Отправляем сообщение о начале процесса обновления
        await query.message.reply_text(
            "✏️ Давайте обновим ваш профиль бегуна перед созданием нового плана."
        )
        
        # Предлагаем пользователю отправить команду /update вручную
        await query.message.reply_text(
            "Пожалуйста, напишите команду /update, чтобы обновить ваш профиль."
        )
