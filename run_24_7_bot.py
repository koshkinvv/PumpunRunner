#!/usr/bin/env python3
"""
Простой скрипт для запуска бота в режиме 24/7 через webhook.
Этот скрипт должен запускаться через workflow bot_runner.
"""
...
def run_forever():
    """Запускает бота в режиме бесконечного цикла webhook."""
    logging.info("Запуск бота в режиме 24/7...")
    ...
    logging.info("Webhook настроен и активен. Запуск бесконечного цикла...")
    ...
    while True:
        try:
            ...
            # Периодически проверяем статус webhook (каждые 5 минут)
            if time.time() % 300 < 60:  # Проверяем примерно раз в 5 минут
                if not check_webhook_status():
                    logging.warning("Webhook не активен. Переустанавливаем...")
                    setup_webhook(replit_domain)
            
            # Спим 60 секунд перед следующей итерацией
            time.sleep(60)
