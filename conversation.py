...
class RunnerProfileConversation:
    ...
    async def collect_goal(self, update: Update, context: CallbackContext):
        ...
        if text == 'Улучшить время':
            ...
            # Переходим сразу к вопросу о комфортном темпе бега
            reply_markup = ReplyKeyboardMarkup(
                [
                    ['4:30 - 5:30'],
                    ['5:30 - 6:30'],
    ...
    # Метод collect_fitness был удален, т.к. вопрос о физической подготовке был убран
    ...
    async def confirm_data(self, update: Update, context: CallbackContext):
        ...
        if text == 'Да, сохранить мой профиль':
            ...
            if DBManager.save_runner_profile(user_id, profile_data):
                ...
                # Проверяем, первый ли раз пользователь создает профиль
                is_update = context.user_data.get('is_profile_update', False)
                
                # Если это не обновление, а первый раз - задаем вопрос об оплате
                if not is_update:
                    reply_markup = ReplyKeyboardMarkup(
                        [
                    ...
                    # Если это обновление профиля, просто показываем сообщение об успехе
                    await update.message.reply_text(
                        "🎉 Отлично! Ваш профиль бегуна успешно обновлен. "
